...
def _get_coords_dataarrays_from_index(container):
  ...
  for coord in [k for k in container.coords if k not in container.base_coords]:
    ...
    xr_coord = pd.Series(container._get_coord_data_from_index(coord), index=pd.MultiIndex.from_tuples(list(zip(*tuple(container._get_coord_data_from_index(dep) for dep in deps))), names=list(map(str, deps))))
    try:
...
def _check_series_unique(series):
  def check_unique(group):
    assert len(group.drop_duplicates()) == 1, "Data not uniquely indexed for base coords: ({})".format(group.name if isinstance(group.name, string_types) else ','.join(group.name))
...
def _append_coords_to_dataset(ds, container, base_only, attrs=None):
  ...
  for coord in container.coords:
    ...
    if len(data.index.names) > len(container.coords._base_dependencies[coord]):
      data.reset_index([c for c in data.index.names if c not in container.coords._base_dependencies[coord]], inplace=True, drop=True)
    ds.coords[str(coord)] = metacsv_series_to_dataarray(data, attrs=container.variables.get(coord, {}))
...
def metacsv_dataframe_to_dataarray(dataframe, names=None, attrs=None):
  ...
  dataframe.columns.names = [str(c) if not pd.isnull(c) else 'coldim_{}'.format(i) for i, c in enumerate(dataframe.columns.names)]
