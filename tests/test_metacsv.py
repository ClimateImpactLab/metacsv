...
def test_read_csv(setup_env):
    ...
    csv1 = metacsv.read_csv(os.path.join(testdata_prefix, "test1.csv"))
...
def test_coordinate_conversion_to_xarray(setup_env):
    ...
    df = metacsv.read_csv(os.path.join(testdata_prefix, "test5.csv"), squeeze=True)
...
def test_for_series_attributes(setup_env):
    ...
    s = metacsv.read_csv(
        os.path.join(testdata_prefix, "test5.csv"), squeeze=True, index_col=[0, 1]
    )
...
def test_series_conversion_to_xarray(setup_env):
    ...
    csv1 = metacsv.read_csv(os.path.join(testdata_prefix, "test5.csv"), squeeze=True)
        ...
        assert (
            (
                abs(
                    df.values
                    - ds.to_dataframe()
                    .set_index([i for i in df.coords if i not in df.base_coords])
                    .values
                )
                < 1e-7
            )
...
def test_xarray_variable_attribute_persistence(setup_env):
    testfile = os.path.join(testdata_prefix, "test6.csv")
    assert metacsv.read_csv(testfile).to_xarray().col1.attrs["unit"] == "wigits"
...
def test_standalone_properties(setup_env):
    ...
    variables = metacsv.core.internals.Variables(
        {"column1": {"units": "wigits"}, "column2": {"units": "wigits"}}
    )
...
def test_parse_vars(setup_env):
    ...
    df = metacsv.read_csv(
        os.path.join(testdata_prefix, "test7.csv"), parse_vars=True, index_col=0
    )
    ds = df.to_xarray()
...
def test_assertions(setup_env):
    ...
    df = metacsv.read_csv(
        fp, parse_vars=True, assertions={"attrs": {"version": "test5.2016-05-01.01"}}
    )
    ...
    df = metacsv.read_csv(
        fp, parse_vars=True, assertions={"variables": {"col2": {"unit": "digits"}}}
    )
...
def test_header_writer(setup_env):
    ...
    coords = {"ind1": None, "ind2": None, "ind3": "ind2"}
    variables = {
        "col1": dict(description="my first column"),
        "col2": dict(description="my second column"),
    }
